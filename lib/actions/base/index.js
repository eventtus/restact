...
function createBase() {
  var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _config2.default,
      toastr = _ref.toastr,
  ...
  function wrapRequest(config) {
    ...
    request.then(onSuccess).then(function () {
      ...
      var message = _objectPath2.default.get(thrown, ['response', 'headers', 'x-error'], thrown.message);

      if (hasLoader) loadingComplete(dispatch);

      if (!_axios2.default.isCancel(thrown) && toastr) {
        toastr.show({
          type: 'error',
          title: 'Error',
          text: message,
